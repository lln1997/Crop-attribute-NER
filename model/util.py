...
def tensorized(batch, maps):
    ...
    batch_tensor = torch.ones(batch_size, max_len).long() * PAD  # 扩成每句都一样长
    for i, l in enumerate(batch):
        for j, e in enumerate(l):
            batch_tensor[i][j] = maps.get(e, UNK)  # 如果指定键不存在，返回默认值
    # batch各个元素的长度
...
def sort_by_lengths(word_lists, tag_lists):
    pairs = list(zip(word_lists, tag_lists))  # 压缩成元组组成的列表，如[(['你', '我'], [1, 2]), (['他'], [0])]
    indices = sorted(range(len(pairs)),
                     key=lambda k: len(pairs[k][0]),
                     reverse=True)  # 降序 [(['你', '我'], [1, 2]), (['他'], [0])]该例会返回[0,1] 一维的
    pairs = [pairs[i] for i in indices]
    # pairs.sort(key=lambda pair: len(pair[0]), reverse=True)

    word_lists, tag_lists = list(zip(*pairs))  # 解压
...
def cal_loss(logits, targets, tag2id):
    ...
    参数:
        logits: [B, L, out_size]
        targets: [B, L] 此例[64,178]
        lengths: [B]
    """
    PAD = tag2id.get('<pad>')
    assert PAD is not None

    mask = (targets != PAD)  # [B, L](布尔类型的矩阵)
    targets = targets[mask]  # 一维列表 把[B,L]的targets展成去掉PAD之后的一维列表 此例为[8827]
    out_size = logits.size(2)  # 第三维的数值
    a = mask.unsqueeze(2).expand(-1, -1, out_size)
    logits = logits.masked_select(
        mask.unsqueeze(2).expand(-1, -1, out_size)  # 把mask添加第三维，转成和logits一样维度的矩阵
    ).contiguous().view(-1, out_size)  # 此例压缩为[8827,30]，-1的作用是定了out_size，第一维自动填充

    assert logits.size(0) == targets.size(0)
    # logits是[8827,30]，每个字都有30个标签的概率，取最大概率的那个和targets做交叉熵全部相加为Loss
